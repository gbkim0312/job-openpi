...
`ACTIVE`는 출처의 명시적 활성 상태 또는 유효한 지원 동작으로 확인된 경우만 사용합니다. 명시적 마감은 `CLOSED`, 삭제/404는 `DELETED`입니다. 완료된 전체 검색에서 공고가 사라졌을 때도 저장된 마감일이 오늘보다 이전이면 `CLOSED`로 확정합니다. 마감일이 남아 있거나 마감일이 없는 경우에는 기존 DB 상태를 유지하며, 단순 미등장을 이유로 `UNKNOWN`으로 바꾸지 않습니다.
...
완료된 출처 검색에서 기존 공고가 보이지 않으면 최초 수집 시 저장된 공고 정보를 기준으로 재검증합니다. 저장된 `deadline_date`가 현재 날짜보다 이전이면 `CLOSED`로 변경 이력을 남깁니다. 마감일이 미래이거나 없는 경우에는 기존 DB 상태를 유지하고 `last_checked_at`만 갱신합니다. 프로필 일부만 실행한 검색에서는 다른 프로필 공고의 상태를 변경하지 않습니다.
