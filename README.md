...
대시보드 **설정**에서 Cron 형식(분 시 일 월 요일)으로 검색 동기화와 재확인 일정을 변경할 수 있습니다. 예약 동기화는 등록된 모든 프로필의 출처별 검색어를 합쳐 출처마다 순차 실행하며, 중복 공고는 한 번만 처리합니다. 설정은 PostgreSQL에 저장되며 Scheduler 컨테이너가 최대 30초 안에 반영합니다.
